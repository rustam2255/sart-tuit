...
const FacultyCard = ({
  ...
  return (
    <div
      onClick={() => {
        navigate(`/all-faculties/${id}`)
        localStorage.setItem("faculty_id", id);
      }}
      className="faculty-card cursor-pointer rounded-[20px] bg-white z-0 shadow-[4px_4px_8px_0px_#1E1E1E14] inter pb-10"
    >
      <div
      ...
      >
        ...
        <div className="faculty-card__logo absolute bottom-0 translate-y-1/2 right-[1.5rem] rounded-full bg-white">
          <img
            src={icon}
            alt={`${name} logo`}
            className="h-[6.75rem] w-[6.75rem] object-fit object-cover rounded-full"
          />
        </div>
      </div>
